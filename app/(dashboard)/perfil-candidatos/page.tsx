...
export default function PerfilCandidatosPage() {
  ...
  const alternarVisto = async (candidato: CandidatoItem, novoVisto: boolean) => {
    if (!candidato) return
    const id = candidato.id
    const token = candidato.token_acesso

    // Atualização otimista no estado local
    setCandidatos((prev) =>
      prev.map((c) => {
        if ((id && c.id === id) || (token && c.token_acesso === token)) {
          return { ...c, perfil_visto: novoVisto }
        }
        return c
      })
    )

    // Persistência na tabela perfil_candidatos via API
    try {
      await fetch("/api/candidatos-perfil", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id,
          token,
          action: "alternar_visto",
          visto: novoVisto
        })
      })
    } catch (err) {
      console.error("Erro ao registrar status visto no banco:", err)
    }
  }
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0 flex-wrap">
                        ...
                        {c.status === "concluido" && (() => {
                          const candKey = c.id || c.token_acesso
                          const jaVisto = Boolean(c.perfil_visto)
                          return (
                            <div className="flex items-center gap-1.5">
                              <button
                                onClick={() => {
                                  if (!jaVisto) {
                                    alternarVisto(c, true)
                                  }
                                  setCandidatoSelecionadoId(candKey)
                                  setAbaAtiva("analise")
                                }}
                                className={cn(
                                  "flex items-center gap-1.5 px-4 py-2 rounded-xl text-white text-[11px] font-bold uppercase tracking-wider transition-all shadow-xs cursor-pointer",
                                  jaVisto
                                    ? "bg-emerald-600 hover:bg-emerald-700 shadow-emerald-600/20"
                                    : "bg-[#0F172B] hover:bg-slate-800"
                                )}
                                title={jaVisto ? "Perfil já visto (clique para ver novamente)" : "Ver Perfil Completo"}
                              >
                                {jaVisto ? (
                                  <>
                              ...
                              {jaVisto && (
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    alternarVisto(c, false)
                                  }}
                                  className="flex items-center gap-1 px-2.5 py-2 rounded-xl border border-slate-200 bg-white hover:bg-rose-50 text-slate-600 hover:text-rose-600 text-xs font-semibold transition-colors cursor-pointer shadow-xs"
                                  title="Desfazer: Reverter para não visto (caso clicado por engano)"
                                >
                                  <Undo2 className="w-3.5 h-3.5" />
                                  <span className="text-[11px] font-bold">Desfazer</span>
                                </button>
                              )}
                            </div>
                          )
                ...
                <div className="pt-2 pb-1">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {candidatoAtivo.perfil_visto && (
                        <button
                          type="button"
                          onClick={() => alternarVisto(candidatoAtivo, false)}
                          className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200 transition-colors cursor-pointer"
                          title="Clique para desfazer / desmarcar 'Já Visto' deste candidato"
                        >
                          <Undo2 className="w-3 h-3" />
                          <span>Desmarcar 'Já Visto'</span>
                        </button>
                      )}
                    </div>
