...
export default function FinancialSettingsPage() {
    ...
    try {
      ...
      for (const item of bannerForm.images) {
        ...
        if (item.file) {
          ...
          const isProportional = Math.abs(aspectRatio - 1.777) < 0.25
          if (!isProportional) {
            console.warn(`Imagem de proporção não-padrão detectada: ${img.width}x${img.height} (Aspecto: ${aspectRatio.toFixed(2)})`)
            toast.warning(`A imagem "${item.file.name}" não está no formato ideal de 16:9. Mas não se preocupe! Ela foi mantida e se ajustará ao carrossel.`)
          }
  ...
  const handleOpenEditBanner = (banner: DashboardBanner) => {
  ...
  const handleBannerEditSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    ...
    try {
      ...
      if (editBannerForm.imageFile) {
        console.log("Iniciando upload de nova imagem...")
        const img = new window.Image()
        const objectUrl = URL.createObjectURL(editBannerForm.imageFile)
        ...
        if (!isProportional) {
          console.warn(`Imagem de proporção não-padrão detectada: ${img.width}x${img.height} (Aspecto: ${aspectRatio.toFixed(2)})`)
          toast.warning(`A nova imagem não está no formato ideal de 16:9, mas foi mantida e se ajustará ao carrossel.`)
        }
        URL.revokeObjectURL(objectUrl)

        const fileExt = editBannerForm.imageFile.name.split('.').pop()
        const fileName = `${Math.random().toString(36).substring(2)}.${fileExt}`
        const filePath = `banners/${fileName}`
        ...
        const { error: uploadError } = await withRetry(() => 
          supabase.storage
            .from('dashboard-banners')
            .upload(filePath, editBannerForm.imageFile!)
        )
      ...
      const { error: updateError } = await withRetry(() =>
        supabase
          .from('dashboard_banners')
          .update({
            title: editBannerForm.title || null,
      ...
      toast.error(`Erro ao atualizar o banner: ${errorMsg}`)
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const toggleRegraAtiva = async (prodId: string, regras: ProdutoRegra[], ruleIndex: number) => {
    try {
      const updatedRegras = (regras || []).map((r, i) => {
        if (i === ruleIndex) {
          const currentAtivo = r.ativo !== false
          return { ...r, ativo: !currentAtivo }
        }
        return r
      })
      ...
      const { error } = await supabase
        .from('produtos_config')
        .update({ regras: updatedRegras })
        .eq('id', prodId)

      if (error) throw error
      toast.success("Status do coeficiente atualizado")
      fetchStatuses()
    } catch (error) {
      console.error("Erro ao alternar status de coeficiente:", error)
      toast.error("Erro ao alternar status de coeficiente")
    }
  }
  ...
  const handleSaveProdutoConfig = async () => {
    ...
    try {
      ...
      // Validamos e formatamos todas as regras da lista tempRegras
      const formattedRegras = tempRegras
        .map(r => ({
      ...
      const payload = {
        ...
        prazo: (formattedRegras.length > 0 && formattedRegras[0].prazo) ? parseInt(formattedRegras[0].prazo) : null,
        coeficiente: (formattedRegras.length > 0 && formattedRegras[0].coeficiente) ? parseFloat(formattedRegras[0].coeficiente) : null,
        percentual_producao: (formattedRegras.length > 0 && formattedRegras[0].percentual_producao) ? parseFloat(formattedRegras[0].percentual_producao) : null
      }
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f8fafc]">
      ...
      <Dialog open={isAddOperacaoModalOpen} onOpenChange={setIsAddOperacaoModalOpen}>
        <DialogContent className="sm:max-w-[800px] border-none rounded-3xl shadow-2xl overflow-y-auto max-h-[90vh]">
          <DialogHeader>
            <DialogTitle className="text-[14px] font-extrabold text-slate-800 tracking-widest uppercase">
          ...
          <div className="space-y-6 pt-4">
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              {/* Lista Dinâmica de Campos das Regras */}
              <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
                {tempRegras.length === 0 ? (
                  <div className="text-center py-6 border border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                    <span className="text-[11px] font-medium text-slate-400">Nenhuma regra adicionada. Clique em Adicionar Regra para começar.</span>
                  </div>
                ) : (
                  tempRegras.map((regra, index) => (
                    <div key={index} className="relative p-4 bg-slate-50/50 rounded-2xl border border-slate-100 space-y-3 group transition-all hover:bg-slate-50">
                      <div className="flex items-center justify-between pb-1 border-b border-slate-100/60">
                        <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">Regra #{index + 1}</span>
                        {tempRegras.length > 1 && (
                          <Button 
                            type="button" 
                            variant="ghost" 
                            ...
                            className="h-6 w-6 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-md transition-colors"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </Button>
                        )}
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div className="space-y-1">
                          ...
                          <Input 
                            ...
                            onChange={(e) => {
                              ...
                              newRegras[index] = { ...newRegras[index], prazo: e.target.value }
                        ...
                        <div className="space-y-1">
                          <Label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest pl-0.5">Coeficiente</Label>
                          <Input 
                            type="text"
                            value={regra.coeficiente}
                            onChange={(e) => {
                              const newRegras = [...tempRegras]
                              newRegras[index] = { ...newRegras[index], coeficiente: e.target.value }
                              setTempRegras(newRegras)
                            }}
                        ...
                        <div className="space-y-1">
                          <Label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest pl-0.5">Produção (%)</Label>
                          <Input 
                            type="text"
                            value={regra.percentual_producao}
                            onChange={(e) => {
                              const newRegras = [...tempRegras]
                              newRegras[index] = { ...newRegras[index], percentual_producao: e.target.value }
                              setTempRegras(newRegras)
                            }}
                        ...
                        <div className="space-y-1">
                          <Label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest pl-0.5">Comissão (%)</Label>
                          <Input 
                            type="text"
                            value={regra.percentual_comissao}
                            onChange={(e) => {
                              const newRegras = [...tempRegras]
                              newRegras[index] = { ...newRegras[index], percentual_comissao: e.target.value }
                              setTempRegras(newRegras)
                            }}
      ...
      <Dialog open={isBannerModalOpen} onOpenChange={setIsBannerModalOpen}>
        <DialogContent className="sm:max-w-[700px] max-h-[85vh] flex flex-col border-none rounded-3xl shadow-2xl p-0 overflow-hidden">
          ...
          <form onSubmit={handleBannerSubmit} className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
              ...
              <div className="space-y-4">
                <Label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest font-extrabold">Imagens do Banner (Proporção 16:9 recomendada)</Label>
            ...
            <DialogFooter className="p-6 border-t border-slate-100 gap-3 bg-slate-50 flex-shrink-0">
              <Button 
                type="button" 
                variant="outline" 
      ...
      <Dialog open={isEditBannerModalOpen} onOpenChange={setIsEditBannerModalOpen}>
        <DialogContent className="sm:max-w-[700px] max-h-[85vh] flex flex-col border-none rounded-3xl shadow-2xl p-0 overflow-hidden">
          ...
          <form onSubmit={handleBannerEditSubmit} className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
              <div className="space-y-2">
                <Label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Título da Campanha (Opcional)</Label>
                <Input 
              ...
              <div className="space-y-4">
                <Label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest font-extrabold font-black">Imagem do Banner (Proporção 16:9 recomendada)</Label>
                
                <div className="grid grid-cols-2 gap-4">
                  {editBannerForm.imagePreview && (
                    <div className="relative aspect-[2560/1440] rounded-2xl overflow-hidden border-2 border-primary/20 shadow-lg group">
                      <img src={editBannerForm.imagePreview} className="w-full h-full object-cover" alt="Preview Banner" />
                    </div>
                  )}
                  ...
                  <label className="flex flex-col items-center justify-center aspect-[2560/1440] border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 hover:bg-slate-100/50 hover:border-primary/30 transition-all cursor-pointer">
                    <div className="flex flex-col items-center justify-center p-4 text-center">
                      <Plus className="w-6 h-6 text-slate-300 mb-2" />
                      <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Substituir Imagem</p>
                    </div>
                    <input 
                      type="file" 
                      className="hidden" 
            ...
            <DialogFooter className="p-6 border-t border-slate-100 gap-3 bg-slate-50 flex-shrink-0">
              ...
              <Button 
                ...
                className="px-8 h-[40px] bg-[#171717] hover:bg-[#171717]/90 text-white font-black text-[10px] rounded-xl gap-3 transition-all uppercase tracking-widest shadow-xl shadow-slate-200 min-w-[160px]"
              >
                {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4" /> Salvar Alterações</>}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
